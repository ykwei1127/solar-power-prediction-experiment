...
for i in range(0, len(train_data)):
    ...
    for j in range(0, len(train_data[0])):
        if i == 0:
            temp.append(train_data[i][j] - train_data[i][j])
        else:
            temp.append(train_data[i][j] - train_data[i-1][j])
...
for i in range(0, len(test_data)):
    if i == 0:
        test_temp.append(test_data[i] - test_data[i])
    else:
        test_temp.append(test_data[i] - test_data[i-1])
